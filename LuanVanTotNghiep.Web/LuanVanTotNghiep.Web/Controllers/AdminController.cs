...
using LuanVanTotNghiep.Web.Common;
...
namespace LuanVanTotNghiep.Web.Controllers
{
	...
	public class AdminController : ControllerBase
	{
		...
		[HttpPost]
		public async Task<IActionResult> PostAdmin([FromForm] AccountDTO admin)
		{
			...
			{
				var IsImage = ImagesPath.IsImage(admin.ImageAvatar);
				if (IsImage == true)
				{
					var uniqueFileName = ImagesPath.GetUniqueFileName(admin.ImageAvatar.FileName);
					var uploads = Path.Combine(_host.WebRootPath, "ImagesAdmin");
					var filePath = Path.Combine(uploads, uniqueFileName);
					admin.ImageAvatar.CopyTo(new FileStream(filePath, FileMode.Create));
					admin.UrlAvatar = ImagesPath.PathImages("ImagesAdmin/", uniqueFileName);
				}
				else
				{
		...
		[HttpPut("{id}")]
		public async Task<IActionResult> PutAdmin([FromForm] AccountDTO admin)
		{
