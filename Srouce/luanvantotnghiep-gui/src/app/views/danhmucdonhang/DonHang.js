...
export default function DonHang() {
	...
	//set thanh toán roi
	const SetThanhToan = id => {
		setConfirmDialog({
			...confirmDialog,
			isOpen: false
		})
		createAPIEndpoint(ENDPIONTS.DONDATHANG + "/checkthanhtoan").fetchById(id)
	...
	const SetDaGiaoDonHang = id => {
		...
		createAPIEndpoint(ENDPIONTS.DONDATHANG + "/setdonhangdagiao").fetchById(id)
			...
			.catch(err => setNotify({
				isOpen: true,
				message: 'Đã giao không thành công',
				type: 'error'
			}));
	}
	const {
										...
										listdonhang.length > 0 ?
											listdonhang.map(data => (
												<TableRow key={data.id}>
													...
													{
														data.trangThai == true ?
															<TableCell className={classes.ColorCheck} align="center">Đã thanh toán</TableCell>
															: <TableCell className={classes.ColorGiaHang} align="center">Chứa thanh toán</TableCell>
													}
													{data.daGiao == false ?
															...
															data.daGiao == true ? "" :
																<Controls.ActionButton
																	...
																	onClick={() => {
																		setConfirmDialog({
																			isOpen: true,
																			title: 'Bạn có chắc chắn là đơn hàng này đã giao rồi hay không',
																			subTitle: "Bạn không thể hoàn tác thao tác này",
																			onConfirm: () => { SetDaGiaoDonHang(data.id) }
																		})
																	}}>
																	<CheckSharp className={classes.ColorCheck} fontSize="small" />
																</Controls.ActionButton>
														}
														{/* Đánh giấu đã thanh toán */}
														{
															data.trangThai == true ? "" :
																<Controls.ActionButton
																	color="primary"
																	onClick={() => {
																		setConfirmDialog({
																			isOpen: true,
																			title: 'Bạn có chắc chắn là đơn hàng này đã thanh toán rồi không',
																			subTitle: "Bạn không thể hoàn tác thao tác này",
																			onConfirm: () => { SetThanhToan(data.id) }
																		})
																	}}>
																	<PaymentSharp className={classes.ColorCheckThanhToan} fontSize="small" />
																</Controls.ActionButton>
														}
														{/* nút xem chi tiết */}
														<Controls.ActionButton
